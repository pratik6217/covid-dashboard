...
# Writitin the code for Home Function.
	def home(self):
		...
		if self.option2 == 'Summary':
			self.uu = "https://covid-19-coronavirus-statistics2.p.rapidapi.com/totalData"
			self.headers = {
				'x-rapidapi-host': "covid-19-coronavirus-statistics2.p.rapidapi.com",
				'x-rapidapi-key': "1b1dc259d1msh6d0cb315bc85403p13231cjsn2cf13404bcd6"
				}

			self.response = requests.request("GET", self.uu, headers= self.headers).json()
			self.t = {"Labels": ['Confirmed', 'Deaths', 'Recovered'],
			"Values": [self.response['result']['totalCases'],
						self.response['result']['totalDeaths'],
						self.response['result']['totalRecovered']]}
			self.dd = pd.DataFrame(self.t)
			# st.write(self.dd)
			self.c = self.response['result']['totalCases']
			self.d = self.response['result']['totalDeaths']
			self.r = self.response['result']['totalRecovered']
			self.tes = self.c.split(",")
			self.c = "".join(self.tes)
			self.tes = self.d.split(",")
			self.d = "".join(self.tes)
			self.tes = self.r.split(",")
			self.r = "".join(self.tes)
			#st.write(self.df.style.background_gradient(cmap = 'Reds'))
			try:
				st.write('')
				self.des1 = st.columns(3)
				self.des1[0].subheader("Confirmed:")
				self.des1[1].subheader("Deaths:")
				self.des1[2].subheader("Recovered:")
				self.des2 = st.columns(3)
				self.c1 = f'<h1 style= "color:grey;">{self.c}</h1>'
				self.des2[0].markdown(self.c1, unsafe_allow_html = True)
				self.d1 = f'<h1 style= "color: tomato;">{self.d}</h1>'
				self.des2[1].markdown(self.d1, unsafe_allow_html = True)
				self.r1 = f'<h1 style= "color: green;">{self.r}</h1>'
				self.des2[2].markdown(self.r1, unsafe_allow_html = True)
				st.write('')

				st.write("")
				st.subheader("Pie Chart:")
				self.pie_fig = px.pie(self.df, values= self.df['Values'] , names= self.df['Labels'], title= 'Summary of Covid - 19 Cases.') 
				st.write(self.pie_fig)
				st.write()
				st.subheader('Histogram:')
				# st.write(f' ⚫ Confirmed: {self.response["result"]["totalCases"]}')
				# st.write(f' ⚫ Recovered: {self.rec}')
				# st.write(f' ⚫ Deaths: {self.dece}')
				# st.write(f' ⚫ Testing: {self.test}')
				# self.des = st.columns(2)
				# self.des[0].write(f'Confirmed:\n{type(self.c)}')
				self.histogram_fig = px.histogram(x = ['Confirmed', 'Deaths', 'Recovered'], y = [self.c, self.d, self.r]).update_layout(xaxis_title = 'Labels',yaxis_title = "Number of Cases")
	...
	def raw(self):
		...
		st.write(' ')
		self.option = st.selectbox('Menu', ['Cases in India', 'Compare Data' ])
		if self.option == 'Cases in India':
			st.write(" ")
			# self.uri = 'https://api.covid19api.com/live/country/India'
			self.u = 'https://api.apify.com/v2/key-value-stores/toDWvRj1JpTXiM8FF/records/LATEST?disableRedirect=true'
			self.d = requests.get(self.u).json()
			...
			st.write(f' ⚫ Active Cases: {self.d["activeCases"]}')
			st.write(f' ⚫ Total Cases: {self.d["totalCases"]}')
			st.write(f' ⚫ Recovered: {self.d["recovered"]}')
			st.write(f' ⚫ Deaths: {self.d["deaths"]}')
			st.write(f' ⚫ Previous Day Tests: {self.d["previousDayTests"]}')
			st.write(f' ⚫ Source: {self.d["sourceUrl"]}')
			
			# self.da = st.date_input('Enter a date:')
			# self.date = str(self.da) + ' 00:00:00+00:00'
			# #st.write(self.date)
